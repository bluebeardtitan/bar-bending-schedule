...
/* =========================
   Bar weight synopsis (per-diameter)
   Every bar's weight (totalWtKg) is pooled by its diameter and rolled up into a
   single figure per Ø, plus an overall total. Wt/m is derived from the pooled
   figures (kg per metre of total length), so it reflects the actual schedule.
   The grand total is quoted in metric tonnes (MT) — how reinforcement steel is
   procured — while per-diameter weights stay in kg to match the schedule table.
   ========================= */
function computeDiaWeight(rowList){
  const byDia = new Map();
  (rowList||[]).forEach(r=>{
    const q = Number(r.qty)||0, tLen = Number(r.totalLenM)||0, tWt = Number(r.totalWtKg)||0;
    if(q<=0 || tWt<=0) return;
    if(!byDia.has(r.dia)) byDia.set(r.dia, { dia:r.dia, barCount:0, totalLenM:0, totalWtKg:0 });
    const g = byDia.get(r.dia);
    g.barCount += q; g.totalLenM += tLen; g.totalWtKg += tWt;
  });
  const groups = [...byDia.values()].sort((a,b)=>a.dia-b.dia);
  groups.forEach(g=>{ g.unitWt = g.totalLenM ? g.totalWtKg/g.totalLenM : 0; });
  const total = groups.reduce((a,g)=>({
    barCount:a.barCount+g.barCount, totalLenM:a.totalLenM+g.totalLenM,
    totalWtKg:a.totalWtKg+g.totalWtKg,
  }), { barCount:0, totalLenM:0, totalWtKg:0 });
  total.unitWt = total.totalLenM ? total.totalWtKg/total.totalLenM : 0;
  groups.forEach(g=>{ g.wtPct = total.totalWtKg ? (g.totalWtKg/total.totalWtKg)*100 : 0; });
  return { groups, total };
}
...
/* Bar weight synopsis — always computed over the full schedule (not the
   filtered/paged view), since a procured steel order covers every bar. */
function renderDiaWeight(){
  const body = $('#diaWeightBody'); if(!body) return;
  const { groups, total } = computeDiaWeight(rows);
  body.innerHTML = groups.length
    ? groups.map(g=>`
      <tr>
        <td class="mono right">${g.dia}</td>
        <td class="mono right">${g.barCount}</td>
        <td class="mono right">${fmt3(g.totalLenM)}</td>
        <td class="mono right">${fmt3(g.unitWt)}</td>
        <td class="mono right">${fmt3(g.totalWtKg)}</td>
        <td class="mono right">${g.wtPct.toFixed(1)}%</td>
      </tr>`).join('')
    : `<tr><td colspan="6" class="right" style="text-align:center;color:var(--muted)">No bars yet</td></tr>`;
  $('#dwBars').textContent  = total.barCount;
  $('#dwLen').textContent   = fmt3(total.totalLenM);
  $('#dwKg').textContent    = fmt3(total.totalWtKg);
  $('#dwMT').textContent    = fmt3(total.totalWtKg/1000);
  $('#dwPct').textContent   = '100%';
}
...
function toCSV(){
  ...
  // ── Bar weight synopsis (per diameter, total in metric tonnes) ──
  const { groups: wg, total: wt } = computeDiaWeight(rows);
  if (wg.length) {
    lines.push('');
    lines.push('BAR WEIGHT SYNOPSIS (per diameter; total in metric tonnes)');
    const wData = wg.map(g => ({
      'Ø (mm)': g.dia,
      'No. of Bars': g.barCount,
      'Total L (m)': fmt3(g.totalLenM),
      'Wt/m (kg)': fmt3(g.unitWt),
      'Total Wt (kg)': fmt3(g.totalWtKg),
      '% of total': `${g.wtPct.toFixed(1)}%`,
    }));
    wData.push({
      'Ø (mm)': 'Total', 'No. of Bars': wt.barCount, 'Total L (m)': fmt3(wt.totalLenM),
      'Wt/m (kg)': '', 'Total Wt (kg)': fmt3(wt.totalWtKg), '% of total': '100%',
    });
    lines.push(Papa.unparse(wData, { newline: '\r\n' }));
    lines.push(`Total weight (metric tonnes),${fmt3(wt.totalWtKg/1000)} MT`);
  }
...
$('#printBBS').addEventListener('click', async () => {
  ...
  try {
    ...
    // ── Bar weight synopsis (per diameter, total in metric tonnes) ──
    const { groups: wGroups, total: wTot } = computeDiaWeight(rows);
    if (wGroups.length) {
      ...
      pdf.text('Bar Weight Synopsis  (per diameter, total in metric tonnes)',
               margin, y, { baseline:'top' });
      y += lineH + 4;

      const wc = [
        { t:'Ø (mm)', w:26 }, { t:'Bars', w:26 }, { t:'Total L (m)', w:36 },
        { t:'Wt/m (kg)', w:36 }, { t:'Total Wt (kg)', w:40 }, { t:'%', w:22 },
      ];
      ...
      wGroups.forEach(g => {
        ...
        const vals = [g.dia, g.barCount, fmt3(g.totalLenM), fmt3(g.unitWt), fmt3(g.totalWtKg), `${g.wtPct.toFixed(1)}%`];
        wc.forEach((c,i) => wCell(c, vals[i], y, false));
        pdf.setLineWidth(0.06); pdf.line(margin, y + wRowH, margin + wTableW, y + wRowH);
        y += wRowH;
      });

      const tv = ['Total', wTot.barCount, fmt3(wTot.totalLenM), '', fmt3(wTot.totalWtKg), '100%'];
