...
function strokeRebarPath(ctx, path, samples, diam, cfg) {
  ...
  /* Vector capture (PDF export): stroke the centreline polyline only — a
     clean schematic bar. Glossy highlight (needs shadow) and rib texture
     are screen-only eye-candy and are intentionally dropped so the vector
     output stays small and print-clean. */
  if (ctx.__vector) {
    if (samples.length >= 2) {
      ctx.save();
      ...
      ctx.strokeStyle = GRAY.body; ctx.lineWidth = d; ctx.stroke();
      ctx.restore();
    }
    return;
  }
...
/* =====================================================
   VECTOR EXPORT
   A minimal canvas2d-compatible recording context turns the
   existing drawing code (dim renderers, shape-library diagrams,
   rebar strokes) into a flat list of vector primitives. The result
   is normalised into a 0..1 box so the PDF exporter can re-draw it
   as true vector graphics (crisp, tiny) instead of a raster image.
   ===================================================== */
function createVectorRecorder() {
  const ops = [];
  let m = [1, 0, 0, 1, 0, 0];          // current transform matrix (a,b,c,d,e,f)
  const stack = [];
  let st = { stroke:'#000', fill:'#000', lineWidth:1, dash:[],
             font:'10px sans', align:'start', baseline:'alphabetic' };
  let subs = [];     // completed subpaths (device coords) for current path
  let cur  = [];     // current subpath points (device coords)
  let lu   = null;   // last point in USER coords (for curve/arc flattening)

  const apply = (x, y) => [ m[0]*x + m[2]*y + m[4], m[1]*x + m[3]*y + m[5] ];
  const sx = () => Math.hypot(m[0], m[1]);   // device scale (x axis)
  const rot = () => Math.atan2(m[1], m[0]);
  const fontPx = () => { const mm = /([\d.]+)px/.exec(st.font); return mm ? parseFloat(mm[1]) : 10; };
  const isWhite = c => /^#fff(fff)?$/i.test(String(c)) || /rgb\(\s*255\s*,\s*255\s*,\s*255/i.test(String(c));
  ...
  function flushSub() { if (cur.length > 1) subs.push(cur); cur = []; }
  function emitStroke() {
    flushSub();
    if (subs.length) ops.push({ k:'stroke', paths:subs, w:st.lineWidth*sx(), color:st.stroke, dash:st.dash.map(v=>v*sx()) });
    subs = []; cur = [];
  }
  function emitFill() {
    flushSub();
    if (subs.length) ops.push({ k:'fill', paths:subs, color:st.fill });
    subs = []; cur = [];
  }
  ...
  const ctx = {
    ...
    set lineCap(_){}, get lineCap(){ return 'round'; },
    set lineJoin(_){}, get lineJoin(){ return 'round'; },
    set shadowColor(_){}, set shadowOffsetX(_){}, set shadowOffsetY(_){},
    set shadowBlur(_){}, set globalAlpha(_){}, set lineDashOffset(_){},
    save(){ stack.push({ m:m.slice(), st:Object.assign({}, st, {dash:st.dash.slice()}) }); },
    ...
    beginPath(){ subs = []; cur = []; lu = null; },
    closePath(){ if (cur.length){ cur.push(cur[0].slice()); flushSub(); } },
    moveTo(x,y){ flushSub(); cur = [apply(x,y)]; lu = [x,y]; },
    lineTo(x,y){ if (!cur.length) cur = [apply(x,y)]; else cur.push(apply(x,y)); lu = [x,y]; },
    bezierCurveTo(c1x,c1y,c2x,c2y,x,y){
      const [x0,y0] = lu || [x,y]; const N = 16;
      for (let i=1;i<=N;i++){ const t=i/N,mt=1-t;
        const bx=mt*mt*mt*x0+3*mt*mt*t*c1x+3*mt*t*t*c2x+t*t*t*x;
        const by=mt*mt*mt*y0+3*mt*mt*t*c1y+3*mt*t*t*c2y+t*t*t*y;
        cur.push(apply(bx,by)); }
      lu = [x,y];
    },
    quadraticCurveTo(cx,cy,x,y){
      const [x0,y0] = lu || [x,y]; const N = 12;
      for (let i=1;i<=N;i++){ const t=i/N,mt=1-t;
        const bx=mt*mt*x0+2*mt*t*cx+t*t*x, by=mt*mt*y0+2*mt*t*cy+t*t*y;
        cur.push(apply(bx,by)); }
      lu = [x,y];
    },
    arc(cx,cy,r,a0,a1,ccw){
      let sweep = a1 - a0;
      if (ccw){ if (sweep > 0) sweep -= 2*Math.PI; } else { if (sweep < 0) sweep += 2*Math.PI; }
      const N = Math.max(6, Math.ceil(Math.abs(sweep)/(Math.PI/24)));
      for (let i=0;i<=N;i++){ const a=a0+sweep*(i/N); const px=cx+Math.cos(a)*r, py=cy+Math.sin(a)*r;
        if (i===0 && !cur.length){ cur=[apply(px,py)]; } else { cur.push(apply(px,py)); } }
      lu = [cx+Math.cos(a1)*r, cy+Math.sin(a1)*r];
    },
    arcTo(x1,y1){ this.lineTo(x1,y1); },   // unused in ctx paths here; safe fallback
    rect(x,y,w,h){ this.moveTo(x,y); this.lineTo(x+w,y); this.lineTo(x+w,y+h); this.lineTo(x,y+h); this.closePath(); },
    stroke(path){ if (path !== undefined) return; emitStroke(); },   // Path2D arg → ignore
    fill(path){ if (path !== undefined) return; emitFill(); },
    fillRect(x,y,w,h){
      const p=[apply(x,y),apply(x+w,y),apply(x+w,y+h),apply(x,y+h),apply(x,y)];
      ops.push({ k:'fill', paths:[p], color:st.fill });
    },
    strokeRect(x,y,w,h){
      const p=[apply(x,y),apply(x+w,y),apply(x+w,y+h),apply(x,y+h),apply(x,y)];
      ops.push({ k:'stroke', paths:[p], w:st.lineWidth*sx(), color:st.stroke, dash:st.dash.map(v=>v*sx()) });
    },
    clearRect(){},
    clip(){}, setTransform(a,b,c,d,e,f){ m=[a,b,c,d,e,f]; }, resetTransform(){ m=[1,0,0,1,0,0]; },
    measureText(t){ return { width: String(t).length * fontPx() * 0.6 }; },
    fillText(t,x,y){
      const [dx,dy] = apply(x,y);
      ops.push({ k:'text', x:dx, y:dy, t:String(t), s:fontPx()*sx(),
                 align:st.align, baseline:st.baseline, rot:rot(), color:st.fill });
    },
    strokeText(){},
    get _ops(){ return ops; },
  };
...
/* stroke a centreline polyline (array of {x,y}) into a recorder */
function _vecStrokeSamples(rec, samples, d) {
  if (!samples || samples.length < 2) return;
  rec.save();
  rec.beginPath();
  rec.moveTo(samples[0].x, samples[0].y);
  for (let i = 1; i < samples.length; i++) rec.lineTo(samples[i].x, samples[i].y);
  rec.strokeStyle = GRAY.body; rec.lineWidth = d; rec.stroke();
  rec.restore();
}

function _renderCmdVector(cmd, rec) {
  window._drawerActiveStyle = cmd.style || activeStyle;
  const d = cmd.diam || getBarSize();

  if (cmd.type === 'rebar-path') {
    const geom = cmd.bezier ? buildSplineGeometry(cmd.points, cmd.closed||false)
                            : buildFilletGeometry(cmd.points, d, cmd.closed||false);
    _vecStrokeSamples(rec, geom.samples, d);

  } else if (cmd.type === 'ortho-bar') {
    const pts = cmd.closed ? [...cmd.points, cmd.points[0]] : cmd.points;
    _vecStrokeSamples(rec, buildOrthoGeometry(pts, d).samples, d);

  } else if (cmd.type === 'rect') {
    const lx=Math.min(cmd.x,cmd.x+cmd.w), rx=Math.max(cmd.x,cmd.x+cmd.w);
    const ty=Math.min(cmd.y,cmd.y+cmd.h), by=Math.max(cmd.y,cmd.y+cmd.h);
    const c=[{x:lx,y:ty},{x:rx,y:ty},{x:rx,y:by},{x:lx,y:by}];
    _vecStrokeSamples(rec, buildOrthoGeometry([...c, c[0]], d).samples, d);

  } else if (cmd.type === 'circle') {
    const steps=Math.max(48,Math.round((cmd.rx+cmd.ry))), pts=[];
    for(let i=0;i<=steps;i++){const a=i/steps*Math.PI*2; pts.push({x:cmd.cx+Math.cos(a)*cmd.rx,y:cmd.cy+Math.sin(a)*cmd.ry});}
    _vecStrokeSamples(rec, pts, d);

  } else if (cmd.type === 'text') {
    rec.font = `bold ${cmd.size||13}px monospace`;
    rec.fillStyle = GRAY.dim; rec.textAlign = 'left'; rec.textBaseline = 'top';
    rec.fillText(cmd.text, cmd.x, cmd.y);

  } else if (cmd.type === 'dim-aligned') {
    drawDimAligned(rec, cmd.p1, cmd.p2, cmd.offset, cmd.label||getAnnot(), cmd.size);
  } else if (cmd.type === 'dim-angular') {
    drawDimAngular(rec, cmd.vertex, cmd.ptA, cmd.ptB, cmd.label||'', cmd.size);
  } else if (cmd.type === 'dim-leader') {
    drawDimLeader(rec, cmd.origin, cmd.elbow, cmd.textPt||null, cmd.label||getAnnot(), cmd.size);

  } else if (cmd.type === 'shape') {
    drawShapeDiagram(rec, cmd.shape, d, _stageW, _stageH);
  }
}

/* Build a normalised vector model of the current drawing (or null). */
function exportToVector() {
  if (!history.length) return null;
  const rec = createVectorRecorder();
  for (const cmd of history) _renderCmdVector(cmd, rec);
  const ops = rec._ops;
  if (!ops.length) return null;

  // Bounds over all geometry + text extents
  let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
  const grow=(x,y)=>{if(x<minX)minX=x;if(x>maxX)maxX=x;if(y<minY)minY=y;if(y>maxY)maxY=y;};
  for (const op of ops) {
    if (op.k==='text') {
      const w=op.t.length*op.s*0.6, h=op.s;
      grow(op.x-w/2,op.y-h/2); grow(op.x+w/2,op.y+h/2);
    } else {
      for (const path of op.paths) for (const p of path) grow(p[0],p[1]);
    }
  }
  if (!isFinite(minX)) return null;
  ...
  const nx=x=>r4((x-minX)/span), ny=y=>r4((y-minY)/span), nl=v=>r4(v/span);

  const outOps = ops.map(op=>{
    if (op.k==='text')
      return { k:'text', x:nx(op.x), y:ny(op.y), t:op.t, s:nl(op.s),
               align:op.align, baseline:op.baseline, rot:+op.rot.toFixed(4), color:op.color };
    const paths = op.paths.map(p=>p.map(pt=>[nx(pt[0]),ny(pt[1])]));
    if (op.k==='stroke')
      return { k:'stroke', paths, w:nl(op.w), color:op.color, dash:(op.dash||[]).map(nl) };
    return { k:'fill', paths, color:op.color };
  });

  return { bw:r4(bw/span), bh:r4(bh/span), ops:outOps };
}
...
window.ShapeDrawer = {
  async open(callback) {
    ...
    document.getElementById('drawerDone').onclick=()=>{
      ...
      dlg.close();
      if(_stageResizeObserver){_stageResizeObserver.disconnect();_stageResizeObserver=null;}
      if(drawerCallback)drawerCallback(exportToPNG(), exportToVector());
      document.removeEventListener('keydown',onKeyDown);
    };
